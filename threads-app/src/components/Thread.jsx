...
const Thread = ({ user, setOpenPopUp, filteredThread, getThreads, setInteractiveThread }) => {
 ...
 /*---- open modal for 'replies icon' & 'comments icon' ----*/

  const handleClick = () => {
    setOpenPopUp(true)
    setInteractiveThread(filteredThread) 
  }
  ...
  const postLike = async () => {
    const isAlreadyLikedByUser = filteredThread.likes.some( like => like.user_uuid === user.user_uuid )

    if (!isAlreadyLikedByUser) {
      filteredThread.likes.push({ user_uuid: user.user_uuid })

      try {
        // put request to an individual Thread
          ...
          getThreads()
          /*
          The getThreads function in the Thread component is passed as a prop 
          from the parent component (Feed component). 
          It is used to trigger a refresh or update of the threads after 
          a user likes a particular thread.
          -> ensure consistency between the client-side UI and the server-side data
          */
        ...
        <div className="text-container">
          <div>
            ...
            <div>
              ...
              <p className="sub-text">{filteredThread.text}</p>
            </div>
          </div>
